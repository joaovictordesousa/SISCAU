...
<section class="container">
                ...
                @foreach ($historico as $histo)
                    <tr>
                        ...
                        <td style="display: flex; gap: 10px;">
                            <a href=" {{ route('editardados', ['GuiasRecolhimento' => $histo->id]) }} "
                                class="btn btn-warning btn-sm" title="Editar Guia de Recolhimento"><svg
                            ...
                            <a href=" {{ route('principal.show', ['GuiasRecolhimento' => $histo->id]) }} "
                                class="btn btn-primary btn-sm" title="Visualizar Guia de recolhimento"><svg
                                    xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            ...
                            <a class="btn btn-success"
                                href="{{ route('baixas.criar', ['GuiasRecolhimento' => $histo->id]) }}"
                            ...
                            <form action="{{ route('principal.destroy', ['GuiasRecolhimento' => $histo->id]) }}"
                                onsubmit="return confirm('Tem certeza que deseja excluir?')" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger btn-sm"
                                    title="Deletar Guia de recolhimento" style="padding: 8px;">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
    ...
    <div class="container_botoes">
        <a href="{{ route('principal.gerapdf', [$historico]) }}" class="btn btn-primary"
            style="padding: 12px 20px;">Imprimir</a>
